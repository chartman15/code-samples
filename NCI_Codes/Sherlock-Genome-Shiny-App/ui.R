...
#############################################################################################################################################################################################
tabPanel(
  value = "tmb_comparison_tab",
  "TMB Comparison",
  hr(style = "border-top: 1px solid #ffffff;"),
  
  # Instructions and Inputs Box
  box(
    width = 12,
    title = 'Instructions and Inputs',
    status = 'primary',
    solidHeader = TRUE,
    collapsed = FALSE,
    collapsible = TRUE,
    ...
    conditionalPanel(
      condition = "input.tmb_sp_group == true",
      selectInput("tmb_group", label = "Select a group:", choices = NULL, multiple = FALSE)
    ),
    
    h4("Input a name to label the project's cohort on the plot. Otherwise, it will be labeled 'Cohort'."),
    textInput("tmb_cohort_name", "Cohort Name", value = "Cohort", placeholder = "Project Name or Group"),
    
    actionButton("tmb_plot_generate", "Generate TMB Plot", class = 'action-buttons-app')
  ),
  
  # Ensures the image is responsive
  tags$head(tags$style(type = "text/css", "#figure_pdf_tmb img {max-width:100%; width:100%; height:auto}")),
  
  # # Show the TMB Plot (this will be populated by renderImage)
  # uiOutput("figure_pdf_tmb", width = "50%", height = "50%"),
  
  # Show the TMB Plot (this will be populated by renderUI)
  div(
    style = "width:50%; height:50%; margin:auto;",
    uiOutput("figure_pdf_tmb")
  ),
  
  
  # Download button (only shown after clicking "Generate TMB Plot")
  conditionalPanel(
    condition = "input.tmb_plot_generate",
    downloadButton("download_tmb_plot", "Download Plot", class = 'action-buttons-app')
  )
),
#############################################################################################################################################################################################
