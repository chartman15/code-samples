...
# NEW CODE TMB Comparison - CH 
####################################################################################################################
  # deletes temporary PDFs from the www/Genomic Data/Sherlock_TCGA folder when the user closes the app  
  
  observeEvent(input$tmb_plot_generate, {
    print('here now')
    
    # Set project code and cohort name
    project_code <- if (is.null(input$project_code)) 'User_Project' else input$project_code
    cohortName <- if (is.null(input$tmb_cohort_name) || input$tmb_cohort_name == "") 'Cohort' else input$tmb_cohort_name
    filename <- paste0(project_code, '_', cohortName, '_tmb.pdf')
    ...
    # Wait for the plot to be generated
    wait_time <- 0
    while (!file.exists(filename) && wait_time < 5) {
      Sys.sleep(0.5)
    ...
    if (file.exists(filename)) {
      print("File generated! Rendering...")
      
      # Render the PDF in an iframe (correct path)
      output$figure_pdf_tmb <- renderUI({
        tags$iframe(
          style = "width:100%; height:600px; border:none;",
          src = file.path("Genomic Data/Sherlock_TCGA", filename)
        )
      })
      
      # Correct Cleanup: Remove duplicate paths!
      sherlock_path <- normalizePath(
        file.path("www", "Genomic Data", "Sherlock_TCGA", filename),
        mustWork = FALSE
      )
      
      print(paste("Looking for file at:", sherlock_path))
      
      if (file.exists(sherlock_path)) {
        file.remove(sherlock_path)
        print(paste("Deleted file:", filename))
      } else {
        print(paste("File not found for deletion:", sherlock_path))
      }
      
    } else {
      print("Plot generation failed — no file found.")
    }
  })
  
  # Cleanup PDFs when app closes
  session$onSessionEnded(function() {
    print("App closing — cleaning up PDFs...")
    
    # Path to the folder where PDFs are stored
    pdf_folder <- file.path("www", "Genomic Data", "Sherlock_TCGA")
    ...
    if (length(pdf_files) > 0) {
      ...
      print(paste("Deleted PDFs:", pdf_files))
    } else {
      print("No PDFs found for cleanup.")
    }
  })
########################################################################################################################
...
# OLD CODE SCNA Inspect Data - CH
############################################################################################
      #load("./www/Genomic Data/Sherlock_TCGA/SCNA/BBprofile.RData")
############################################################################################

# NEW CODE SCNA Inspect Data - CH     
############################################################################################
      load(paste0(parent,'/www/Genomic Data/Sherlock_TCGA/SCNA/BBprofile.RData')) 
############################################################################################
