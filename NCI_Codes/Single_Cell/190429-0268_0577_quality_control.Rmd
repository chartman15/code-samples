---
title: "190429-0268_0577_quality_control"
...
```{r}
# Read in 10x hdf5 files for both samples (returns sparse matrix)

counts_0268 <- Read10X_h5('data_190429-0268/raw_feature_bc_matrix.h5',use.names = TRUE, unique.features = TRUE) # tumor 

counts_0577 <- Read10X_h5('data_190429-0577/raw_feature_bc_matrix.h5',use.names = TRUE, unique.features = TRUE) # normal

# Can read in the filtered matrices as well 

#counts_0268 <- Read10X_h5('data_190429-0268/filtered_feature_bc_matrix.h5',use.names = TRUE, unique.features = TRUE)

#counts_0577 <- Read10X_h5('data_190429-0577/filtered_feature_bc_matrix.h5',use.names = TRUE, unique.features = TRUE)
...
#c0268 <- CreateSeuratObject(counts = counts_0268) 
#c0577 <- CreateSeuratObject(counts = counts_0577)

c0268 <- CreateSeuratObject(counts = counts_0268, min.features = 100) 
c0577 <- CreateSeuratObject(counts = counts_0577, min.features = 100)
...
head(c0268@meta.data)
head(c0577@meta.data)
...
merged_seurat <- merge(x = c0268, y = c0577, add.cell.id = c("c0268", "c0577"))
...
# in the above example, ctrl = control and stim = stimulated. 
...
metadata$sample <- NA
metadata$sample[which(str_detect(metadata$cells, "^c0268_"))] <- "c0268"
metadata$sample[which(str_detect(metadata$cells, "^c0577_"))] <- "c0577"
