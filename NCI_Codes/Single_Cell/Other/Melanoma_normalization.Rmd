...
Scale the data. Can also scale with SCTransform() (see below).

```{r}
# Scale the counts: used to remove unwanted sources of variation; for example, technical variation introduced during the experiment such as batch effects, or biological variation such as variation due to the cell cycle stage, or mitochondrial contamination. 

# Scaling ensures that when we cluster the cells later on by similarity of gene expression, they will cluster due to the actual differences in gene expression and not these other unwanted sources of variation.
...
This is dimensionality reduction which summarizes gene expression data into principal components (PCs). Cells with similar expression profiles will cluster together. 
...
DimPlot(seurat_phase, reduction = "pca") + NoLegend()
```
...
Determine number of PCs we should keep. Use a scree/elbow plot. The purpose of a scree plot is to visualize the “elbow” in the curve and select all components just before the line flattens out. The first PC usually explains most of the variability in the dataset. 
...
PCA plot: mitochondrial expression. 
...
*Clustering: first without integrating*

FindNeighbors() computes K-nearest neighbors of each cell using specified numbers of PCs. 
FindClusters() performs graph clustering and assigns each cell a number according to its respective cluster.

```{r}

n.pcs <- 13
seed.use <- 123
res.used <- c(0.1) #0.3, 0.5, 0.7, 1)

seurat_phase <- FindNeighbors(seurat_phase,dims=1:n.pcs)
seurat_phase <- FindClusters(seurat_phase,resolution=res.used)
DimPlot(seurat_phase,label=TRUE,pt.size=0.1,group.by="sample")

```

*UMAP: first without integrating* 
...
```{r}
# Run UMAP
seurat_integrated <- RunUMAP(seurat_integrated, dims = 1:13, reduction = "pca", seed.use = 123)

# Plot UMAP                             
DimPlot(seurat_integrated, label = TRUE) 

# Plot UMAP split by sample
DimPlot(seurat_integrated, split.by = "sample", label = TRUE)
```
...
DimPlot(harmonized_seurat, label = TRUE)
