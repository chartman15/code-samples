...
# Paper: Prognostic genome and transcriptome signatures in colorectal cancers (PMID 39112715)

# Data: Supp Table 13. Activities of de novo and Cosmic decomposed mutational signatures by patient tumour. 
#       Supp Table 11. Constitution and standard error of the mean for de novo mutational signatures by mutation type.
#       Supp Table 12. Cosmic decomposed de novo mutational signatures.

library(tidyverse)
library(readxl)

setwd("C:/Users/hartmancas/Desktop/mSigPortal/mSigPortal_Data_Cleaning/UK100k-CRC-N2024")

# load collected mutational signatures, including different COSMIC signatures
load('signature_refsets.RData')


### Supp Table 13: Signature activity data--------------------------------------------------------------------------------------------------------------------------------------------

TableData <- read_xlsx(path = 'Supplementary_Table_13.xlsx',skip = 4,col_names = T)
colnames(TableData)[1] <- "Sample"

# denovo SBS96
sdata <- TableData %>% select(Sample,SBS96A:SBS96AA)

Exposure_refdata_tmp1 <- sdata %>% 
  pivot_longer(cols = -Sample,names_to = "Signature_name",values_to = 'Exposure') %>% 
  mutate(Study = 'UK100k-CRC-N2024', Dataset = 'WGS', Cancer_Type="Colorectal_Carcinoma", Organ="Colon",Signature_set_name = 'Denovo_SBS96') %>% 
  select(Study,Dataset,Cancer_Type,Organ,Sample,Signature_set_name,Signature_name,Exposure)

# denovo DBS78
sdata <- TableData %>% select(Sample,DBS78A:DBS78H)

Exposure_refdata_tmp2 <- sdata %>% 
  pivot_longer(cols = -Sample,names_to = "Signature_name",values_to = 'Exposure') %>% 
  mutate(Study = 'UK100k-CRC-N2024', Dataset = 'WGS', Cancer_Type="Colorectal_Carcinoma", Organ="Colon",Signature_set_name = 'Denovo_DBS78') %>% 
  select(Study,Dataset,Cancer_Type,Organ,Sample,Signature_set_name,Signature_name,Exposure)

# denovo ID83
sdata <- TableData %>% select(Sample,ID83A:ID83K)

Exposure_refdata_tmp3 <- sdata %>% 
  pivot_longer(cols = -Sample,names_to = "Signature_name",values_to = 'Exposure') %>% 
  mutate(Study = 'UK100k-CRC-N2024', Dataset = 'WGS', Cancer_Type="Colorectal_Carcinoma", Organ="Colon",Signature_set_name = 'Denovo_ID83') %>% 
  select(Study,Dataset,Cancer_Type,Organ,Sample,Signature_set_name,Signature_name,Exposure)

# decomposed SBS96
sdata <- TableData %>% select(Sample,SBS1:`SBS-CRC2`)

Exposure_refdata_tmp4 <- sdata %>% 
  pivot_longer(cols = -Sample,names_to = "Signature_name",values_to = 'Exposure') %>% 
  mutate(Study = 'UK100k-CRC-N2024', Dataset = 'WGS', Cancer_Type="Colorectal_Carcinoma", Organ="Colon",Signature_set_name = 'COSMIC_v3.2_Signatures_GRCh38_SBS96_Denovo') %>% 
  select(Study,Dataset,Cancer_Type,Organ,Sample,Signature_set_name,Signature_name,Exposure)

# decomposed DBS78
sdata <- TableData %>% select(Sample,DBS2:`DBS-CRC5`)

Exposure_refdata_tmp5 <- sdata %>% 
  pivot_longer(cols = -Sample,names_to = "Signature_name",values_to = 'Exposure') %>% 
  mutate(Study = 'UK100k-CRC-N2024', Dataset = 'WGS', Cancer_Type="Colorectal_Carcinoma", Organ="Colon",Signature_set_name = 'COSMIC_v3.2_Signatures_GRCh38_DBS78_Denovo') %>% 
  select(Study,Dataset,Cancer_Type,Organ,Sample,Signature_set_name,Signature_name,Exposure)

# decomposed ID83
sdata <- TableData %>% select(Sample,ID1:`ID-CRC2`)

Exposure_refdata_tmp6 <- sdata %>% 
  pivot_longer(cols = -Sample,names_to = "Signature_name",values_to = 'Exposure') %>% 
  mutate(Study = 'UK100k-CRC-N2024', Dataset = 'WGS', Cancer_Type="Colorectal_Carcinoma", Organ="Colon",Signature_set_name = 'COSMIC_v3.2_Signatures_GRCh38_ID83_Denovo') %>% 
  select(Study,Dataset,Cancer_Type,Organ,Sample,Signature_set_name,Signature_name,Exposure)
...
### Supp Table 11: mutational signature data----------------------------------------------------------------------------------------------------------------------------------------------- 

TableData <- read_xlsx(path = 'Supplementary_Table_11.xlsx',skip = 3,col_names = T,range = "A4:AB100",.name_repair = 'check_unique')
...
TableData <- read_xlsx(path = 'Supplementary_Table_11.xlsx',skip = 3,col_names = T,range = "BD4:BL82",.name_repair = 'check_unique')
colnames(TableData)[1] <- "MutationType"
...
TableData <- read_xlsx(path = 'Supplementary_Table_11.xlsx',skip = 3,col_names = T,range = "BU4:CF87",.name_repair = 'check_unique')
colnames(TableData)[1] <- "MutationType"
...
save(exposures_refdata, file='UK100k-CRC-N2024_WGS_exposure_refdata.RData') # study name, dataset and exposure_refdata.RData

signatures_refsets <- signature_refsets_tmp_final
save(signatures_refsets, file='UK100k-CRC-N2024_WGS_signature_refsets.RData') # study name, dataset and signature_refsets.RData

seqmatrix_refdata <- seqmatrix_refdata_tmp_final
save(seqmatrix_refdata, file='UK100k-CRC-N2024_WGS_seqmatrix_refdata.RData') # study name, dataset and seqmatrix_refdata.RData
